...
?>
        <main class="flex-1 p-4 sm:p-6 lg:p-8">
            <?php echo Html::form()
                ->post($urlGenerator->generate('dboard.parameters.edit', ['domain' => $parameter->getDomain(), 'name' => $parameter->getName()]))
                ->csrf($csrf)
                ->noValidate()
                ->open(); ?>
                <?php echo Bleet::cardHeader()
                    ...
                    ->title($translator->translate('Parameter', category: 'dboard-modules'))
                    ->primary();
                ?>
