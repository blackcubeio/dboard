...
?>
        ...
        <main class="flex-1 p-4 sm:p-6 lg:p-8">
            <div class="bg-white rounded-lg shadow-sm border border-secondary-200 p-4 mb-6">
                <?php echo Html::form()->get($urlGenerator->generate('dboard.hosts'))->attribute('class', 'flex flex-wrap gap-4 items-center')->open(); ?>
                    <div class="flex-1">
                    ...
                    <div>
                        <?php echo Bleet::a($translator->translate('Create', category: 'dboard-common'), $urlGenerator->generate('dboard.hosts.create'))
                            ->button()
                            ->icon('plus')
                            ->primary()
                            ->render();
            ...
            <?php echo Bleet::cardHeader()
                ->icon('globe-alt')
                ->title($translator->translate('Hosts', category: 'dboard-modules'))
                ->primary();
            ?>

            <div class="bg-white rounded-b-lg shadow-lg p-4">
                <bleet-ajaxify id="hosts-list">
                <?php
                    echo $this->render('Hosts/_list', [
