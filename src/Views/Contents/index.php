...
$nodeOptions = RouteHelper::create()->getContentRoutes(byId: true, query: Content::query());
...
?>
        ...
        <main class="flex-1 p-4 sm:p-6 lg:p-8">
            ...
            <div class="bg-white rounded-lg shadow-sm border border-secondary-200 p-4 mb-6">
                <?php echo Html::form()->post($urlGenerator->generate('dboard.contents'))->csrf($csrf)->attribute('class', 'flex flex-wrap gap-4 items-center justify-end')->open(); ?>
                    <?php echo Bleet::select()
                        ->active($filterNodeForm, 'nodeId')
                        ->placeholder($translator->translate('-- All contents --', category: 'dboard-content'))
                        ->options($nodeOptions)
