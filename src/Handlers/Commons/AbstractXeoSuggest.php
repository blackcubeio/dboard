...
abstract class AbstractXeoSuggest extends AbstractAjaxHandler
{
    ...
    private function suggest(): void
    {
        ...
        if ($webPageSchema === null) {
            $this->formModel->setTitle(ContentHelper::cleanTitle($model->getName()));
            return;
        }
        ...
        $mappingsByRegularId = [];
        /** @var SchemaSchema $ss */
        foreach ($schemaSchemasQuery->each() as $ss) {
            $mappingsByRegularId[$ss->getRegularElasticSchemaId()] = $ss;
        }

        if (empty($mappingsByRegularId) === true) {
            $this->formModel->setTitle(ContentHelper::cleanTitle($model->getName()));
            return;
        }
        ...
        if ($found === false) {
            ...
            foreach ($pivotsQuery->each() as $pivot) {
                ...
                if ($blocSchemaId !== null && isset($mappingsByRegularId[$blocSchemaId]) === true) {
    ...
    private function applySuggestMapping(SchemaSchema $schemaSchema, object $source, array $webPageToXeo): void
    {
        ...
        if (isset($decoded['mapping']) === false || is_array($decoded['mapping']) === false) {
            return;
        }

        foreach ($decoded['mapping'] as $sourceField => $targetField) {
            if (isset($webPageToXeo[$targetField]) === false) {
                continue;
            }
            $sourceValue = $source->$sourceField ?? null;
            if ($sourceValue !== null && $sourceValue !== '') {
                $sourceValue = ContentHelper::cleanTitle($sourceValue);
